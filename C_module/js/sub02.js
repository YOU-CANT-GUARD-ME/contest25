...
buyBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
        ...
        fetch(`./addCart.php?id=${itemId}`)
            .then((res) => res.text())
            .then((data) => alert(data));
    });
});

$$(".item-price .price").forEach((priceEl) => {
    const priceNum = parseInt(priceEl.textContent.replace(/[^0-9]/g, ""));
    const originalPrice = parseInt(
        priceEl.closest(".item-price")
        .querySelector("span:nth-child(2)")
        .textContent.replace(/[^0-9]/g, "")
    );

    const discounts = {
        1: originalPrice - 10000,
        2: originalPrice * 0.9,
        3: originalPrice * 0.7
    };

    if (discounts[priceNum]) {
        priceEl.textContent = discounts[priceNum].toLocaleString("en-us");
    }
});